...
while True:
    ...
    for i in results:
        ...
        for box in boxes:
            ...
            if curclass=="person"  and conf>0.3:
    ...
    for result in resultsTracker:
        ...
        if limitsUp[0]<cx<limitsUp[2] and limitsUp[1]-15<cy<limitsUp[1]+15:
            if tot_countUp.count(id)==0:
                tot_countUp.append(id)
                cv2.line(img, (limitsUp[0], limitsUp[1]), (limitsUp[2], limitsUp[3]), (0, 255, 0), 4)

        if limitsDown[0]<cx<limitsDown[2] and limitsDown[1]-15<cy<limitsDown[1]+15:
            if tot_countDown.count(id)==0:
                tot_countDown.append(id)
                cv2.line(img, (limitsDown[0], limitsDown[1]), (limitsDown[2], limitsDown[3]), (0, 255, 0), 4)
    #
    # # cvzone.putTextRect(img, f'{len(tot_count)}', (50,50))
    cv2.putText(img,str(len(tot_countUp)),(929,345),cv2.FONT_HERSHEY_PLAIN,5,(139,195,75),7)
    cv2.putText(img, str(len(tot_countDown)), (1191, 345), cv2.FONT_HERSHEY_PLAIN, 5, (50, 50, 230), 7)
